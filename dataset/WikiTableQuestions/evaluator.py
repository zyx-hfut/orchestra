...
def eval_json_multiple():
    ...
    with open(args.prediction_path, 'r', 'utf8') as fin, open(
            args.prediction_path.replace('.json', '_eval_details.tsv'), 'w', 'utf-8') as fout:
        # fout.write('qid\tresults\n')
        assert '.json' in args.prediction_path
        logs = json.load(fin)
        fout.write(
            'id\tdefault\trow_sort\tcol_pull_front\trow_reorder_overlap\trow_reorder_embedding\tcol_reorder_overlap\tcol_reorder_embedding\tmax_sim_col\tmax_lcs_col\n')
        for q in logs:
            # line = line.rstrip('\n').split('\t')
            ex_id = q['id']
                ...
                if 'all_df_permutation_results' in q:
                    for reason in q['all_df_permutation_results']:
                        ...
                        if 'original df' in reason:
                            ...
                            category_correct['row_reorder_embedding'] = category_correct[
                                                                            'row_reorder_embedding'] or correct
                            ...
                            category_correct['col_reorder_embedding'] = category_correct[
                                                                            'col_reorder_embedding'] or correct
...
def eval_json():
    ...
    with open(args.prediction_path, 'r', 'utf8') as fin, open(
            args.prediction_path.replace('.json', '_eval_details.tsv'), 'w') as fout:
        assert '.json' in args.prediction_path
        logs = json.load(fin)
        for q in logs:
            ex_id = q['id']
...
def eval_json_substring():
    ...
    with open(args.prediction_path, 'r', 'utf8') as fin, open(
            args.prediction_path.replace('.json', '_eval_details.tsv'), 'w', 'utf8') as fout:
        assert '.json' in args.prediction_path
        logs = json.load(fin)
        for q in logs:
            ex_id = q['id']
    ...
    with open(args.prediction_path, 'r', 'utf8') as fin, \
        ...
        for q in logs:
            ...
            if ex_id not in target_values_map:
                ...
                if correct:
                    fout.write(ex_id + '\t' + target_values[
                        0].normalized + '\t' + value + '\t' + 'TRUE' + '\t' + '%d' % cnt + '\n')
                else:
                    fout.write(ex_id + '\t' + target_values[
                        0].normalized + '\t' + value + '\t' + 'FALSE' + '\t' + '%d' % cnt + '\n')
                num_examples += 1
                if correct:
                    num_correct += 1
                if top_k_correct:
