...
# TODO: agentscope has implemented the memory module, prompt2msg should be rather simple
def prompt2messages(prompt):
    """
    In agentscope, outputs of LLMs are automatically managed by memory module
    Here we only need to deal with INPUT (role=user) of LLMs
    """
    message = Msg(
        name='msg',
        role="user",
...
def GptCompletion(agent, prompt):
    def gpt(queue):
        try:
            messages = prompt2messages(prompt)

            # QA agent in agentscope
            output = agent(messages)

            # output.choices[0].text = output.choices[0].message.content.strip('```')
            output['text_tmp'] = output['text'].strip('```')
            if 'Answer:' in output['text_tmp'] and 'Answer: ```' not in output['text_tmp']:
                output['text_tmp'] = output['text_tmp'].replace('Answer:', 'Answer: ```')
            # print(output)
            return output
